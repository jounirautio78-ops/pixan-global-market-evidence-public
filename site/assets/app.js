...
function renderMetrics() {
  ...
  const metrics = [
    { label: l("Avoin maapohja", "Open country base"), value: state.globalBase ? `${state.globalBase.countries.length} / 195` : "— / 195", note: state.globalBase ? l(`${state.globalBase.summary.observedCount} World Bank -havaintoa; ei sähkötupakkamyyntiä`, `${state.globalBase.summary.observedCount} World Bank records; no vaping sales`) : l("Pohjakerros ei ole saatavilla; atlaksen maaluetteloa ei käytetä sen korvikkeena", "Base layer unavailable; the atlas country list is not substituted for it") },
