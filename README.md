...
The full atlas and lender/buyer review page expose six release-locked downloads: English and Finnish versions of a 6-slide brief, an extended 30-slide diligence deck and a 53-row Evidence Register workbook. `scripts/artifact-build/build_bank_package_artifacts.mjs` authors both languages in the controlled `@oai/artifact-tool` runtime, adds a non-empty `[Sources]` speaker-note block to every slide, renders every slide and workbook sheet, and writes the reviewed release lock and manifest. Four active presentation templates and two workbook source-sheet seeds are immutable v17 inputs under `scripts/artifact-build/seeds/v17/`; the retired 12-slide templates remain only as historical internal seeds and are excluded from current generation and release lineage. The builder never treats the current public downloads as its hidden seed. Active seed SHA-256 digests are recorded separately as `templateInputs` and also included in the reviewed input lineage. No private workspace file is an input. `site/data/bank-package-manifest.json` records the release, source date, language, exact reviewed-input and artifact SHA-256 digests, file sizes and slide or row counts. CI does not silently regenerate stale presentations: it fails closed if any reviewed input, active seed, download or lock differs. Before displaying a download link, the browser fetches the file and verifies both its exact byte length and full SHA-256 digest against that manifest.

Release `2026.07.26-25` keeps the bilingual Evidence Registers at 53 reviewed claims. The market evidence contains 79 observations from 23 sources. Its 70 official observations are explicitly separated into 34 market measures across seven countries and 36 Swedish FHM register-structure counts for 2018–2026. The release adds the public New Zealand 29-file manifest, deterministic scope parser, aggregate audit and donor-closure pack. The reproduced 2024 specialist-retailer identified-vaping subtotal is NZD 274,180,410.21: NZD 189,402,451.96 consumables, NZD 84,709,409.85 devices/hardware and NZD 68,548.40 mixed systems. Adjacent and unresolved rows are separately quantified and excluded. New Zealand passes 7/10 donor criteria but remains NOT ACCEPTED because D5 fails and D8 and D10 remain open. The Swedish counts remain structure rather than sales, the donor gate remains 0/3 and no global value is published.
...
The recommended sequence is to evaluate samples and transaction-use terms from ECigIntelligence and Euromonitor in parallel, buy at most one global master unless material non-overlap is demonstrated, and then use a tightly scoped NIQ/Circana POS pilot to validate selected countries. Public list prices are dated page observations. A private indicative Euromonitor quote has been received, but its exact commercial terms are not published and no purchase is authorised. Every option remains subject to the sample, methodology, official-anchor reconciliation and written lender/M&A data-room licence gates in the downloadable workbook.
...
The review page also contains a privacy-safe response-control view for the four recorded commercial routes. It keeps outreach status, received evidence, mandatory gates and scoring readiness separate. The current checkpoint contains four tracked routes, one vendor route with a substantive response, zero vendor samples scored and zero purchase authorisations. Euromonitor supplied a partial Germany workbook sample, generic methodology, blank licence terms and two indicative annual package quotes. The sample establishes the existence of relevant category, value or volume, company or brand and other evaluation fields, but it does not contain the current comparable annual Germany liquid totals required for the public 2023–2024 official-anchor test. Euromonitor states that its e-vapour product covers roughly 75 markets; the exact country-product-year-measure matrix is still missing. The quote indicator is received, but all six mandatory gates remain open, the route is **NOT SCORED** and no purchase, fee or commitment is authorised. Exact prices, licensed values, correspondence and attachments remain outside the public repository.
