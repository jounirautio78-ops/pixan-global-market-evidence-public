...
`source/country-method-route-config.json` and [`source/COUNTRY_METHOD_ROUTE_MAP.md`](source/COUNTRY_METHOD_ROUTE_MAP.md) define the method-control overlay. It is deliberately not described as 195 bespoke market routes: only 23 countries have a reviewed country-specific method plan, 5 have a reviewed official-source lead, 15 have only the regional EU TPD reporting pattern, and 152 remain proxy-only and country-unscoped. Every country row exposes its assignment class, transaction stage, provenance basis, next evidence action and retail/donor boundary.
