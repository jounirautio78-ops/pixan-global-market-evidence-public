# Pixan Patent Value Evidence Center — public site

This repository builds a public, source-linked patent-value evidence centre in which global vaping-market evidence is one controlled input. Its primary objective is to estimate a defensible patent value range without relabelling market size as patent value. GitHub Pages receives **only the reviewed contents of `site/`**. The repository is not a data room and must not contain confidential material.
...
The five-minute review opens with a fail-closed Decision Cockpit derived from the reviewed atlas, market, patent and request-programme JSON. It keeps supported facts, unsupported conclusions and current readiness blockers separate, and remains **HOLD — patent value NOT_COMPUTED** while any mandatory valuation gate remains open. The same view exposes the controlled patent-value bridge, a reproducible Germany calculation waterfall, a deterministic 54-source market ledger and a five-candidate donor acceptance ledger. The Evidence Center adds three explicit publication lanes, the same source-linked D1–D10 donor cockpit and a country-year scenario lab. Missing or invalid scenario inputs return `not_computed`, never zero, and no public global total is calculated until at least three donors plus regional and regulatory-archetype coverage pass. Donor readiness is an evidence input, not the final valuation objective. The waterfall must reconcile the committed formula and all three outputs before displaying them. The freshness ledger keeps retrieval date and latest observation year separate, never treats a download date as proof of substantive currency, and labels the atlas's item-level source dates as undated where they are not recorded.
...
## Fail-closed patent valuation control

`source/patent-history.json` contains the canonical public valuation control. Its stated purpose is **Estimate defensible patent value range / Arvioida puolustettavissa oleva patentin arvon vaihteluväli**. Seven separate output cases and every formula-stage EUR value remain `null/NOT_COMPUTED`. The legacy `ultimatePatentValueEUR` field is enforced only as a null sentinel and is never a target or model output. The seven premise-specific output cases are deliberately non-additive:
...
- past enforcement-claim net present value;
- exit/transaction indication; and
- collateral-recovery value.

The controlled seven-step method is `market evidence → scope-key reconciliation → potentially covered sales → route-specific economic benefit → probability-weighted dated cash flows → non-overlapping risk/cost/tax and present value → separate non-additive output`. After covered sales it branches into prospective RFR/direct use, third-party licensing, past enforcement/damages, and strategic option/barrier value. Potentially infringing sales are used only in the past-enforcement branch; they are not a required input for RFR/direct use, third-party licensing, strategic option/barrier, market-participant, owner-specific, exit or collateral outputs. The present-value convention uses dated, probability-weighted cash flows; the discount rate excludes separately modelled risks, every risk maps once, and no separate time factor is applied before or after PV. Scenario probabilities must sum to one. Independent review is a post-computation release/assurance gate, not a model input. Collateral recovery is a separate lender-specific case requiring recovery premise, perfection, priority, transferability, foreclosure marketability, sale time and sale costs—not value multiplied by a generic haircut.

All seven output-specific valuation gates remain open. `BASIS-AND-SUBJECT` must first lock the exact rights, operative claims, territories, separately evidenced know-how, measurement date, valuation premise, intended use, market-participant versus owner-specific basis and gross/net/tax treatment. The other gates cover rights/title/term, covered-use or enforcement-specific product/claim mapping, attributable covered sales, route-specific economic basis, cash-flow timing and output-specific risk/cost/tax/collectability. A result is not computed until its own source and gate dependencies are evidenced; there is no global circular computation block.

Germany is only possible case-specific evidence for the adjudicated product and claim. Any attempted transfer requires current counsel mapping and procedural-status review; the German record does not establish worldwide coverage, infringement, damages or patent value. The official [IFRS 13 Fair Value Measurement overview](https://www.ifrs.org/issued-standards/list-of-standards/ifrs-13-fair-value-measurement/) is cited solely to define the market-participant, measurement-date exit-price basis. It does not classify the owner-specific, licensing, enforcement, transaction or collateral premises used in this research as IFRS fair value. Licensed vendor values, exact deviations, quotes, terms and private material remain outside the public control.

Patent rights are territorial. For this 2013/2014 family and the European patent granted on 29 May 2019, the ordinary PCT national-phase and EP post-grant validation windows are long past. The present public record does not support adding missing countries through an ordinary route. Any restoration, late-entry or other exceptional route is country- and dossier-specific and requires qualified local patent-counsel verification. The older “22” count means 20 national publications plus one EP publication and one WO publication—not 22 confirmed live rights. The current formal table has 28 country rows, but a row count does not prove current validity or close any of the seven valuation gates. See the official [WIPO PCT national-phase guidance](https://www.wipo.int/en/web/pct-system/national-phase) and [EPO national-validation guidance](https://www.epo.org/en/applying/european/validation).
...
The full atlas and lender/buyer review page expose six release-locked downloads: English and Finnish versions of a 6-slide brief, an extended 30-slide diligence deck and a 62-row Evidence Register workbook. The six files are normally generated **at most once per Asia/Nicosia calendar day**. Release `2026.08.03-44` is an exceptional same-day alignment replacement of v43 so that the dashboard, four decks and two registers share the new patent-value objective; normal once-daily cadence resumes thereafter. The live dashboard may otherwise receive additional reviewed same-day releases while the daily download package keeps its own visible version and snapshot timestamp until the next permitted daily build. `scripts/artifact-build/build_bank_package_artifacts.mjs` enforces that cadence and the narrow v44 replacement rule, authors both languages in the controlled `@oai/artifact-tool` runtime, adds a non-empty `[Sources]` speaker-note block to every slide, renders every slide and workbook sheet, and writes the reviewed release lock and manifest. Four active presentation templates and two workbook source-sheet seeds are immutable v17 inputs under `scripts/artifact-build/seeds/v17/`; the retired 12-slide templates remain only as historical internal seeds and are excluded from current generation and release lineage. The builder never treats the current public downloads as its hidden seed. Active seed SHA-256 digests are recorded separately as `templateInputs` and also included in the reviewed input lineage. No private workspace file is an input. `site/data/bank-package-manifest.json` records the package cadence, release, source date, language, exact reviewed-input and artifact SHA-256 digests, file sizes and slide or row counts. Same-day dashboard input drift is allowed only for an explicitly earlier daily package snapshot; package date drift, artifact drift or missing integrity metadata still fails closed. Before displaying a download link, the browser fetches the file and verifies both its exact byte length and full SHA-256 digest against that manifest.

Dashboard release `2026.08.03-44` changes the primary decision question from market size to premise-specific, defensible patent-value ranges and carries the fail-closed valuation control into the dashboard, four decks and two Evidence Registers. It preserves public-official market controls without publishing licensed values, reconstructable deviations, quotes, terms, correspondence or private filenames. Market evidence remains a bounded input and cannot set any output by itself.

The live market dataset contains 174 observations from 54 sources. The 152 official observations comprise 116 market measures across nine countries and 36 Sweden FHM registration-structure records. The six downloadable English/Finnish lender-package files form the reviewed `2026.08.03-44` replacement snapshot; normal once-daily generation resumes after this release. Canada remains NOT ACCEPTED at 7/10, New Zealand remains NOT ACCEPTED at 7/10 and Germany does not change the donor decision. Each Evidence Register exposes the reviewed public claims without licensed vendor values or reconstructable derivatives. The separate open official-data base covers 195 countries with 578 observed World Bank values; 390 WHO or UN Comtrade route records remain queued and missing. The method-control layer classifies the same 195 countries as 28 reviewed country plans, 0 reviewed source leads, 15 regional EU TPD patterns and 152 country-unscoped proxy routes. None of those method classes or demographic, economic, prevalence-route or trade-route records is eligible as retail sales, so the donor gate remains 0/3 and the global market value remains `null/not_computed`. That donor state is an evidence-readiness input, not a patent-value conclusion.

The same-day live additions keep three new evidence roles separate. Spain now has a byte-reproducible AEAT series for provisional 2025 accrued excise of EUR 36.151 million, 2025 net cash receipts of EUR 29.568 million and 2026 H1 net cash receipts of EUR 21.776 million; all four Model 573 epigraphs are combined, devices are excluded and none of the figures is retail value. Japan now has exact-code 854340000 customs-import CIF value and declared-unit series for 2022–2025, with JPY retained as the source currency and same-year ECB EUR equivalents shown only as secondary analytics. New Zealand adds the official 2024 CPI weights of 0.11% total, 0.01% devices and 0.10% refills, but the weights are mainly HES-derived and do not close D5, D8 or D10. See [`source/SPAIN_AEAT_2025_2026_EXCISE_SERIES.md`](source/SPAIN_AEAT_2025_2026_EXCISE_SERIES.md), [`source/JAPAN_2022_2025_OFFICIAL_CUSTOMS_SERIES.md`](source/JAPAN_2022_2025_OFFICIAL_CUSTOMS_SERIES.md) and [`source/NZ_2024_DONOR_GAP_RESEARCH_2026-08-02.md`](source/NZ_2024_DONOR_GAP_RESEARCH_2026-08-02.md). All three remain outside the donor count and global roll-up.
...
The public site retains every source monetary value in its original currency and shows an EUR equivalent only as a secondary comparison. `source/fx-rates.json` contains 32 reviewed ECB EXR annual-average observations covering the current CAD, JPY, NZD, PLN, SEK and USD records and the 2022–2024 World Bank GDP-per-capita base layer. The ECB quotes each rate as foreign-currency units per euro, so the deterministic formula is `EUR equivalent = original amount / currency units per EUR`. The full published `OBS_VALUE` is used for calculation and only the displayed result is rounded. The 2015 PLN rate is retained as a reviewed reference but is not applied to the UOKiK benchmark because the source does not establish a full-calendar-year period.
...
python scripts/validate_spain_aeat_2025_2026_series.py
python scripts/validate_japan_2022_2025_series.py
...
Within that global architecture, the page retains a verified 20-country planned-route queue for existing aggregate records. The ranking is an operational evidence-acquisition order, not a ranking of market size. Its privacy-safe public ledger marks exactly 12 country routes `sent` and 8 `draft_not_sent`: Australia, Canada, Denmark, Finland, France, Germany, Italy, the Netherlands, Poland, Sweden, the United Kingdom and the United States are the recorded sent routes. A supplementary German BVL annual-sales route was sent on 2026-07-24 under section 25 of the Tobacco Products Ordinance, and a supplementary Polish EU-CEG annual-sales aggregate route was sent on 2026-07-28. Both belong to countries already in the queue and do not change the 12/8 country count. Germany's 2026-07-30 reply asks the authority to limit handling to existing aggregates and standard extracts available without charge; paid processing was withheld and no fee or data has been accepted. Canada's two official method clarifications supplied no new market value and closed D5 and D7 as failed while D10 remains open; no further Statistics Canada follow-up is scheduled. Italy's and Denmark's replies are official negative availability results: Italy identifies public ADM routes, while Denmark identifies a retailer registry that describes market structure but not sales. French Customs supplied annual partner-level border-value and net-mass extracts for vaping-device codes across the 2018–2021 and 2022–2025 nomenclature break and then clarified the cited classification and unit metadata; the supplied extract has no supplementary-unit series and remains a supply-stage trade proxy—not retail market size, annual consumer sales or donor evidence. A sent marker, process state or method clarification does not establish a new market value. The repository itself does not send requests, and the downloadable templates remain visibly `DRAFT — NOT SENT`.
...
The review page includes a bilingual, prioritised shortlist of commercial data that could close identified evidence gaps. It is a procurement decision aid, not market data, a purchase recommendation, a vendor endorsement or evidence of Pixan Oy approval. The one-country Germany extract is recorded as accepted and delivered; no wider-package spend is authorised.

The recommended sequence is to evaluate samples and transaction-use terms from ECigIntelligence and Euromonitor in parallel, select at most one global master unless material non-overlap is demonstrated, and only then consider a tightly scoped NIQ/Circana POS pilot for selected countries. This is a prioritisation model, not wider-package purchase authority. Public list prices are dated page observations. Three private indicative Euromonitor package quotes were received, and the one-country Germany extract was accepted, delivered and audited privately. Exact commercial terms and licensed values are not published. Every wider 25/50/78-country package remains HOLD and unauthorised. The later schema workbook lists 95 geographies without populated country-year values; record-level observed/reported/modelled flags, the exact product bridge, lender/buyer data-room rights and all-in tax, fee, retention and renewal terms remain unresolved.
...
The review page also contains a privacy-safe response-control view for the four recorded commercial routes. It keeps outreach status, received evidence, mandatory gates and scoring readiness separate. The current checkpoint contains four tracked routes, one vendor route with substantive responses and zero scored vendors. Euromonitor's one-country Germany extract was accepted, delivered on 2026-08-03 and audited privately. G1 passes; G2, G3, G5 and G6 fail and G4 remains `not_testable`, so 1/6 gates pass and the route remains **NOT SCORED**. All three preregistered numerical liquid-volume proximity tests passed, but that result does not close product, tax, channel, transaction-stage, source-lineage, record-status, rights or complete-commercial-term bridges. Germany remains outside the donor count, the donor gate remains 0/3 and global value remains `not_computed`. Every wider 25/50/78-country package remains HOLD and unauthorised. Exact prices, licensed values, correspondence and attachments remain outside the public repository.
...
PYTHONDONTWRITEBYTECODE=1 python3 scripts/validate_patent_valuation_control.py
...
PYTHONDONTWRITEBYTECODE=1 python3 scripts/test_patent_valuation_control.py
