...
Dashboard and daily-package release `2026.07.29-35` adds privacy-safe official-request and vendor-status deltas without changing a market total or donor decision. Germany supplied an interim processing notice that requests clearer definitions and separates control, seizure and destruction fields; it raises a possible fee and an 11 August 2026 response deadline but supplies no market data and no fee has been accepted. France confirmed border-value and net-mass metadata for the cited customs classifications, which remain a supply-stage trade proxy rather than retail sales. Denmark reported that it does not collect the requested sales figures and identified only a retailer-register route. Luxembourg reported that the requested release volumes, excise revenue and category splits are unavailable and identified an official registered-price list, which is a price reference rather than a market total. A commercial provider offered a conditional paid Germany extract after the review call; it has not been activated or accepted, none of the six evidence gates passes and the provider remains NOT SCORED. The six downloadable lender-package files now share the reviewed `2026.07.29-35` daily snapshot with the dashboard. The market dataset remains at 84 observations from 24 sources and each Evidence Register exposes 60 reviewed claims. Of the 75 official observations, 39 are market measures and 36 are Sweden FHM registration-structure records. The separate open official-data base still covers 195 countries with 578 observed World Bank values; 390 WHO or UN Comtrade route records remain queued and missing. The method-control layer classifies the same 195 countries as 28 reviewed country plans, 0 reviewed source leads, 15 regional EU TPD patterns and 152 country-unscoped proxy routes. None of those method classes or demographic, economic, prevalence-route or trade-route records is eligible as retail sales, so the donor gate remains 0/3 and the global market value remains `null/not_computed`. No commercial data purchase, fee or subscription is authorised.
